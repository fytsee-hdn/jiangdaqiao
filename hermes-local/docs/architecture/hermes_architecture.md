...
Generated runtime root. Stores sessions, memory, logs, cache, outputs, state, and profile-scoped skill candidates. It may be created or updated automatically by the app, but it cannot define or override permissions.
...
Agent-generated skill summaries are runtime candidates by default:

```text
.hermes/profiles/<profile>/skills/
```

Accepted reusable business skill specs may be promoted only by a later reviewed task into:

```text
hermes-local/src/gsp_hermes/skills/<profile>/
```

Runtime skill candidates cannot automatically become accepted source assets.
